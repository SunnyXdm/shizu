...
interface State {
	user: User | null;
	startUser: (phone: string, country: string) => Promise<boolean>;
	register: (body: {
		firstName: string;
		lastName: string;
		username: string;
	...
	getStartUser: () => Promise<any>;
	verify: (otp: string) => Promise<boolean>;
	login: (password: string) => Promise<void>;
	newChat: (userId: string) => Promise<any>;
...
const initialState: State = {
	user: null,
	startUser: async (phone: string, country: string): Promise<boolean> => false,
	register: async (body: {
		firstName: string;
		lastName: string;
		username: string;
	...
	verify: async (otp: string): Promise<boolean> => false,
	login: async (password: string): Promise<void> => {},
	newChat: async (userId: string): Promise<any> => {},
	getChats: async (): Promise<any> => {},
	getChat: async (chatId: string): Promise<any> => {},
	sendMessage: async (chatId: string, message: Message): Promise<any> => {},
};

// Create Context
const ShizuContext = createContext<State>(initialState);
...
export const ShizuProvider = ({
	...
	const init = async () => {
		...
		console.log('Shizu SDK init', JSON.stringify(user, null, 2));
		return;
	...
	const startUser = async (phone: string, country: string): Promise<boolean> => {
		try {
	...
	const getStartUser = async (): Promise<any> => {
	...
	async function newChat(userId: string): Promise<any> {
		try {
	...
	async function getChat(chatId: string): Promise<any> {
		try {
			const {
				data: { chat },
